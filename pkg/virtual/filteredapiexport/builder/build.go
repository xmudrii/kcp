...
import (
	...
	aeadmission "github.com/kcp-dev/kcp/pkg/virtual/filteredapiexport/admission"
	virtualapiexportauth "github.com/kcp-dev/kcp/pkg/virtual/filteredapiexport/authorizer"
	"github.com/kcp-dev/kcp/pkg/virtual/filteredapiexport/controllers/apireconciler"
	"github.com/kcp-dev/kcp/pkg/virtual/filteredapiexport/schemas"
...
func BuildVirtualWorkspace(
	...
	admissionChain := aeadmission.NewSelectorAdmission(kcpInformers.Apis().V1alpha2().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), kubeClusterClient)
	...
	filteredBoundOrClaimedWorkspaceContent := &virtualdynamic.DynamicVirtualWorkspace{
		...
		BootstrapAPISetManagement: func(mainConfig genericapiserver.CompletedConfig) (apidefinition.APIDefinitionSetGetter, error) {
			...
			apiReconciler, err := apireconciler.NewAPIReconciler(
				kcpClusterClient,
				kcpInformers.Apis().V1alpha2().FilteredAPIExportEndpointSlices(),
			...
			if err := mainConfig.AddPostStartHook(apireconciler.ControllerName, func(hookContext genericapiserver.PostStartHookContext) error {
				...
				for name, informer := range map[string]cache.SharedIndexInformer{
					...
					"filteredapiexportendpointslices": kcpInformers.Apis().V1alpha2().FilteredAPIExportEndpointSlices().Informer(),
				} {
...
func newAuthorizer(kubeClusterClient, deepSARClient kcpkubernetesclientset.ClusterInterface, cachedKcpInformers, kcpInformers kcpinformers.SharedInformerFactory) authorizer.Authorizer {
	maximalPermissionAuth := virtualapiexportauth.NewMaximalPermissionAuthorizer(deepSARClient, kcpInformers.Apis().V1alpha2().FilteredAPIExportEndpointSlices(), cachedKcpInformers.Apis().V1alpha2().APIExports())
	...
	boundApiAuth := virtualapiexportauth.NewBoundAPIAuthorizer(apiExportsContentAuth, kcpInformers.Apis().V1alpha2().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), cachedKcpInformers.Apis().V1alpha2().APIExports(), kubeClusterClient)
